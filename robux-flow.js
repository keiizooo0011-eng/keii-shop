    ...
    const render = (o) => {
      root.innerHTML = `<section class="flow-card payment-flow-card">
        ...
        <div class="payment-flow-grid">
          <div class="payment-qris-panel qris-reveal-panel"><div class="qris-reveal-icon">▦</div><span class="qris-reveal-kicker">METODE PEMBAYARAN</span><h2>Bayar dengan QRIS</h2><p>Kode pembayaran dibuka dalam jendela khusus agar tampil utuh dan mudah dipindai.</p><button id="showPaymentQris" class="primary-btn full qris-open-btn"><span class="qris-open-btn-icon">▦</span><span><b>Tampilkan QRIS</b><small>Buka kode pembayaran</small></span><i>→</i></button><small>Mendukung aplikasi bank dan e-wallet berlogo QRIS.</small></div>
          <div class="payment-info-panel">
            <div class="payment-summary-head"><span>RINGKASAN TOP UP</span><h3>Detail pembayaran</h3><p>Periksa akun dan paket Robux sebelum membayar.</p></div><dl class="flow-detail-list payment-detail-list"><div class="payment-detail-row invoice-row"><dt>Nomor invoice</dt><dd><span class="invoice-value">${esc(o.invoice)}</span><button id="copyPaymentInvoice" class="copy-mini">Salin</button></dd></div><div class="payment-detail-row"><dt>Metode</dt><dd>${esc(o.method_name)}</dd></div><div class="payment-detail-row"><dt>Paket</dt><dd>${esc(o.package_label)}</dd></div><div class="payment-detail-row"><dt>Username</dt><dd>${esc(o.username)}</dd></div><div class="payment-detail-row total-row"><dt>Total pembayaran</dt><dd class="payment-grand-total">${rupiah(o.payment_amount)}</dd></div></dl>

...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'download-ticket') {
      ...
      const publicBase = String(process.env.BOT_CONTROLLER_PUBLIC_URL || env('BOT_CONTROLLER_URL')).replace(/\/$/, '');
      if (!publicBase) return res.status(500).json({ error: 'Alamat publik Bot Controller belum diatur.' });

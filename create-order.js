...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { count, error: stockError } = isManualPanel ? { count: Number(product.stock || 0), error: null } : await db.from("stock_items")
    ...
    try {
      if (paymentMethod === "balance") {
        ...
        const reserved = isManualPanel ? { error: null } : await db.rpc("reserve_order_stock", { p_order_id: order.id });
        ...
        if (debit.error) {
          if (!isManualPanel) try { await db.rpc("release_order_stock", { p_order_id: order.id }); } catch {}
        ...
        balanceDebited = true;
        if (isManualPanel) {
          await db.from("orders").update({ status: "paid", paid_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", order.id);
        } else {

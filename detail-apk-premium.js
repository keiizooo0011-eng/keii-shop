(()=>{
 ...
 const labels={pending:'Menunggu pembayaran',processing:'Sedang diproses',completed:'Pesanan selesai',cancelled:'Dibatalkan',failed:'Gagal'};
 ...
 async function init(){
  ...
  try{const r=await fetch('/api/check-payment?invoice='+encodeURIComponent(invoice),{cache:'no-store'}),d=await r.json();if(!r.ok||!d.order)throw new Error(d.error||'Pesanan tidak ditemukan.');const o=d.order,status=String(o.status||'pending').toLowerCase(),fields=parseDelivery(o.delivery_content);
   root.innerHTML=`<section class="flow-card order-detail-flow premium-order-detail"><div class="flow-card-head"><div><span class="eyebrow">DETAIL PESANAN APK PREMIUM</span><h1>${esc(o.product_name||'APK Premium')}</h1><p>${esc(o.invoice||invoice)}</p></div><span class="flow-status ${esc(status)}">${esc(labels[status]||status)}</span></div><div class="order-detail-grid"><dl class="flow-detail-list premium-detail-list"><dt>Invoice</dt><dd>${esc(o.invoice||invoice)} <button id="copyDetailInvoice" class="copy-mini">Salin</button></dd><dt>Produk</dt><dd>${esc(o.product_name||'APK Premium')}</dd><dt>Paket</dt><dd>${esc(o.variant_name||'Paket utama')}</dd><dt>Total</dt><dd class="payment-grand-total">${money(o.payment_amount??o.amount)}</dd><dt>Dibuat</dt><dd>${esc(date(o.created_at))}</dd><dt>Diperbarui</dt><dd>${esc(date(o.updated_at||o.completed_at))}</dd></dl><aside class="order-note-box"><strong>Status pesanan</strong><p>${status==='completed'?'Pesanan telah selesai dan data produk sudah tersedia.':status==='pending'?'Selesaikan pembayaran agar pesanan diproses otomatis.':'Status pesanan akan diperbarui otomatis.'}</p><a class="secondary-btn full" href="riwayat-apk-premium.html">Kembali ke Riwayat</a>${status==='pending'?`<a class="secondary-btn full" href="payment-order.html?invoice=${encodeURIComponent(o.invoice||invoice)}">Buka Pembayaran</a>`:''}</aside></div>${status==='completed'?`<section class="delivery-receipt payment-delivery-result"><div class="delivery-data-title"><div><span>DATA PRODUK</span><strong>Informasi yang kamu terima</strong></div><span>${fields.length} data</span></div><div class="delivery-fields">${fields.length?fields.map((f,i)=>`<div class="delivery-field"><div><span>${esc(f.label)}</span><strong>${esc(f.value)}</strong></div><button data-copy-value="${esc(f.value)}">Salin</button></div>`).join(''):'<div class="delivery-empty-information">Belum ada data produk yang ditampilkan.</div>'}</div></section>`:''}</section>`;

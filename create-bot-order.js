...
export default async function handler(req, res) {
  ...
  try {
    ...
    const session = await botController('/api/sessions', { method: 'POST', body: {
    ...
    const { data: fresh, error: ue } = await db.from('bot_orders').update({
      status: session.status || 'waiting_pairing', pairing_code: session.pairingCode || null,
      controller_data: session, updated_at: new Date().toISOString()

(()=>{
  ...
  function variantsFromText(){return $('#productVariants').value.split('\n').map(x=>x.trim()).filter(Boolean).map(line=>{const [name,normal,reseller]=line.split('|');return{name:(name||'Paket utama').trim(),normal_price:Number(normal||$('#productNormalPrice').value||$('#productPrice').value||0),reseller_price:Number(reseller||$('#productPrice').value||normal||0)}})}
  ...
  async function saveProduct(){
    ...
    try{
      ...
      await api('/api/admin-reseller',{method:'POST',body:JSON.stringify({action:'product',id:$('#editingId').value||null,name,category:$('#productCategory').value,description:$('#productDescription').value.trim(),image_url,normal_price:+$('#productNormalPrice').value,price:+$('#productPrice').value,stock:+$('#productStock').value,delivery_mode:$('#deliveryMode').value,requires_email:$('#requiresEmail').checked,sort_order:+$('#sortOrder').value,is_active:$('#activeProduct').checked,promo_text:$('#promoText').value.trim(),variants:variantsFromText(),stock_lines:stockFromText()})});
  ...
  function reset(scroll=true){
    ['editingId','productName','productDescription','productNormalPrice','productPrice','productStock','promoText','productVariants','stockData'].forEach(id=>$('#'+id).value='');
  ...
  async function handleClick(e){
    try{
      const edit=e.target.closest('[data-edit]');if(edit){const p=data.products.find(x=>String(x.id)===String(edit.dataset.edit));if(!p)return;$('#editingId').value=p.id;$('#productName').value=p.name;$('#productCategory').value=p.category;$('#productDescription').value=p.description||'';$('#productNormalPrice').value=p.normal_price??p.price;$('#productPrice').value=p.price;$('#productStock').value=p.stock;$('#deliveryMode').value=p.delivery_mode;$('#requiresEmail').checked=!!p.requires_email;$('#sortOrder').value=p.sort_order||0;$('#activeProduct').checked=!!p.is_active;$('#promoText').value=p.promo_text||'';$('#productVariants').value=(p.variants||[]).map(v=>`${v.name}|${v.normal_price??v.price??p.normal_price??p.price}|${v.reseller_price??p.price??v.price}`).join('\n');currentImage=p.image_url||'';$('#imagePreview').src=currentImage||'icons/icon-512.png';$('#stockDataWrap').hidden=p.delivery_mode!=='automatic';$('#productFormTitle').textContent='Edit Produk Reseller';window.scrollTo({top:280,behavior:'smooth'});return;}

...
export default async function handler(req, res) {
  ...
  try {
    ...
    const isManualApk = product.category === "apk-premium" && String(product.delivery_mode || "automatic") === "manual";
    ...
    const { count, error: stockError } = isManualDelivery ? { count: 1, error: null } : await db.from("stock_items")

  ...
  try{
    ...
    if(order.status==='pending'){
      ...
      if(match){
        ...
        const {data:used}=await db.from('robux_orders').select('id').eq('payment_reference',key).neq('id',order.id).maybeSingle();
        if(!used){
...
function publicRobux(o){return {invoice:o.invoice,method_name:o.method_name,package_label:o.package_label,robux_amount:o.robux_amount,amount:o.amount,payment_amount:o.payment_amount,username:o.username,whatsapp:o.whatsapp,status:o.status,expires_at:o.expires_at,paid_at:o.paid_at,created_at:o.created_at,admin_note:o.admin_note||''};}

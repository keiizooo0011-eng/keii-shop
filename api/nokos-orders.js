...
export default async function handler(req,res){
 ...
 try{
  if(req.method==='GET'){
   ...
   const rows=data||[];
   await Promise.all(rows.filter(o=>o.supplier_order_id&&['received','expiring','creating'].includes(o.status)).map(async o=>{try{const r=await rumahOtp('/v1/orders/get_status',{order_id:o.supplier_order_id});const d=r.data||{};const patch={supplier_status:d.status||o.supplier_status,status:normalize(d.status),phone_number:d.phone_number||o.phone_number,otp_code:d.otp_code||null,otp_message:d.otp_msg||null,expires_at:d.expired_at?new Date(Number(d.expired_at)).toISOString():o.expires_at,supplier_raw:r,updated_at:new Date().toISOString()};await db.from('nokos_orders').update(patch).eq('id',o.id);Object.assign(o,patch);}catch{}}));
...
function normalize(s){s=String(s||'').toLowerCase();return ({received:'received',completed:'completed',canceled:'canceled',cancel:'canceled',expiring:'expiring'})[s]||s||'received';}

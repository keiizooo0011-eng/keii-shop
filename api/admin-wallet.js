  ...
  try{
    ...
    if(req.method==='GET'){
      const [authMapResult,profilesResult,depositsResult,transactionsResult]=await Promise.all([
        ...
        db.from('deposits').select('id,user_id,invoice,amount,payment_amount,unique_fee,status,created_at,paid_at,expires_at').order('created_at',{ascending:false}).limit(150),

...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'download-ticket') {
      ...
      // Gunakan alamat controller yang memang sudah dipakai dan terbukti aktif (termasuk port-nya).
      // Tidak membutuhkan BOT_CONTROLLER_PUBLIC_URL tambahan.
      const publicBase = String(env('BOT_CONTROLLER_URL')).replace(/\/$/, '');
      if (!publicBase) return res.status(500).json({ error: 'BOT_CONTROLLER_URL belum diatur.' });
      return res.status(200).json({
        ok: true,
        name,
        expiresAt: body.data?.expires,
        downloadUrl: `${publicBase}${body.data?.path || ''}`
      });
    }
